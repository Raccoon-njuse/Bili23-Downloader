...
使用本仓库根目录的 `./bili23`，不要假定系统 PATH 中存在同名命令。此 Skill 是 Agent 调用本地 CLI 的操作规范，不是网络暴露的 MCP 服务，也不提供或保存新的登录凭据。
...
支持项目级 Skill 的 Agent 应加载本目录的 `SKILL.md`，并通过 shell 在仓库根目录运行 `./bili23`。其他 Agent 框架可将本文件作为项目指令，并复用相同的命令与安全边界。
